import 'package:flutter/material.dart';

/// Code 128 barcode renderer using CustomPainter.
/// Supports Code 128B (alphanumeric ASCII 32-127).
class Code128Painter extends CustomPainter {
  final String value;
  final Color color;
  final Color backgroundColor;

  Code128Painter({required this.value, this.color = Colors.black, this.backgroundColor = Colors.transparent});

  static const Map<int, String> _patterns = {
    0: '212222', 1: '222122', 2: '222221', 3: '121223', 4: '121322',
    5: '131222', 6: '122213', 7: '122312', 8: '132212', 9: '221213',
    10: '221312', 11: '231212', 12: '112232', 13: '122132', 14: '122231',
    15: '113222', 16: '123122', 17: '123221', 18: '223211', 19: '221132',
    20: '221231', 21: '213212', 22: '223112', 23: '312131', 24: '311222',
    25: '321122', 26: '321221', 27: '312212', 28: '322112', 29: '322211',
    30: '212123', 31: '212321', 32: '232121', 33: '111323', 34: '131123',
    35: '131321', 36: '112313', 37: '132113', 38: '132311', 39: '211313',
    40: '231113', 41: '231311', 42: '112133', 43: '112331', 44: '132131',
    45: '113123', 46: '113321', 47: '133121', 48: '313121', 49: '211331',
    50: '231131', 51: '213113', 52: '213311', 53: '213131', 54: '311123',
    55: '311321', 56: '331121', 57: '312113', 58: '312311', 59: '332111',
    60: '314111', 61: '221411', 62: '431111', 63: '111224', 64: '111422',
    65: '121124', 66: '121421', 67: '141122', 68: '141221', 69: '112214',
    70: '112412', 71: '122114', 72: '122411', 73: '142112', 74: '142211',
    75: '241211', 76: '221114', 77: '413111', 78: '241112', 79: '134111',
    80: '111242', 81: '121142', 82: '121241', 83: '114212', 84: '124112',
    85: '124211', 86: '411212', 87: '421112', 88: '421211', 89: '212141',
    90: '214121', 91: '412121', 92: '111143', 93: '111341', 94: '131141',
    95: '114113', 96: '114311', 97: '411113', 98: '411311', 99: '113141',
    100: '114131', 101: '311141', 102: '411131',
    103: '211412', // Start A
    104: '211214', // Start B
    105: '211232', // Start C
    106: '2331112', // Stop
  };

  List<String> _encode() {
    var checksum = 104; // Start B
    final encoded = [_patterns[104]!];

    for (var i = 0; i < value.length; i++) {
      var code128Value = value.codeUnitAt(i) - 32;
      if (code128Value < 0 || code128Value > 102) code128Value = 0;
      encoded.add(_patterns[code128Value]!);
      checksum += code128Value * (i + 1);
    }

    final checksumValue = checksum % 103;
    encoded.add(_patterns[checksumValue]!);
    encoded.add(_patterns[106]!); // Stop

    return encoded;
  }

  @override
  void paint(Canvas canvas, Size size) {
    final bgPaint = Paint()..color = backgroundColor;
    canvas.drawRect(Offset.zero & size, bgPaint);

    final encoded = _encode();
    var totalUnits = 0;
    for (final pattern in encoded) {
      for (var j = 0; j < pattern.length; j++) {
        totalUnits += int.parse(pattern[j]);
      }
    }

    final unitWidth = size.width / totalUnits;
    final barPaint = Paint()..color = color;

    var x = 0.0;
    for (final pattern in encoded) {
      for (var j = 0; j < pattern.length; j++) {
        final barWidth = int.parse(pattern[j]) * unitWidth;
        if (j % 2 == 0) {
          canvas.drawRect(Rect.fromLTWH(x, 0, barWidth, size.height), barPaint);
        }
        x += barWidth;
      }
    }
  }

  @override
  bool shouldRepaint(covariant Code128Painter oldDelegate) =>
      value != oldDelegate.value || color != oldDelegate.color;
}

class Barcode128 extends StatelessWidget {
  final String value;
  final double width;
  final double height;
  final Color color;
  final Color backgroundColor;

  const Barcode128({
    super.key,
    required this.value,
    this.width = 300,
    this.height = 80,
    this.color = Colors.black,
    this.backgroundColor = Colors.white,
  });

  @override
  Widget build(BuildContext context) {
    return Container(
      width: width,
      height: height,
      color: backgroundColor,
      child: CustomPaint(painter: Code128Painter(value: value, color: color, backgroundColor: backgroundColor)),
    );
  }
}
